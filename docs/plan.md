...
UIデザインおよび静的フロントエンドの制作（デザインは基本的にカスタムCSS（通常のCSS）で実装し、Salesforce標準デザイン（SLDS v1）は利用可能な状態を維持する）を行い、LWC化担当の別ベンダーへ引き継ぎを行う作業が主となるプロジェクト。

対象画面は15ページあり、画面ごとに複数のコンポーネントを準備する必要がある比較的大規模な構成となる。

## カスタムCSS実装・LWC化を前提とした構成検討
...
1. カスタムCSSを基本とした（SLDS v1は必要時のみ部分利用）静的HTML / CSS / JavaScriptで画面またはパーツを作成する
...
- デザインは基本的にカスタムCSS（通常のCSS）で実装し、SLDSは基本的に利用しない。ただしSLDS v1は利用可能な状態にしておき、必要な場合に限り部分的に利用してよい（利用する場合はSLDS v1準拠とする）
...
## 6. デザインは基本的にカスタムCSSで実装し、SLDS v1は利用可能な状態にしておく

静的HTMLのデザインは、SLDS v1のBlueprintを基準にするのではなく、基本的にカスタムCSS（通常のCSS）で実装する。

SLDS v1自体の利用を禁止するものではない。SLDS v1のCSS/リソースはプロジェクトから参照できる状態にしておき、SLDSを使ったほうが合理的な場合など、必要な画面・コンポーネントに限り部分的に利用してよい。ただし全画面・全コンポーネントで一律に利用することは想定しない。

例えばボタンは、基本的に以下のようにカスタムCSSで実装する。

```html
<button class="app-button app-button_brand" type="button">保存</button>
```
...
SLDS v1を部分的に利用する場合は、以下のようにSLDSクラスをそのまま利用する（独自クラスによる直接上書きは行わない。詳細は「7. SLDSを利用する場合はSLDSクラスを独自CSSで直接上書きしない」を参照）。
...
## 7. SLDSを利用する場合はSLDSクラスを独自CSSで直接上書きしない

デザインは基本的にカスタムCSSで実装するため、この項目が問題になる場面は限定的だが、SLDS v1を部分的に利用するコンポーネントについては、以下のようなSLDSクラス自体への上書きは原則禁止とする。
...
slds-*
    ↓
SLDSを利用する場合、Salesforce / SLDS側の責務

app-*
    ↓
プロジェクト独自実装（基本はこちらでデザインを組む）
```
...
SLDS v1を部分的に利用している箇所については、SLDS Blueprintにアクセシビリティ要件が記載されている場合はそれも実装する。WCAG 2.1 AAの達成基準を満たしているかを最終確認の観点とする。
...
5. デザインは基本的にカスタムCSSで実装し、SLDS v1は利用可能な状態にしておく（利用時はSLDS v1準拠とする）
6. LWC化後はLightning Base Componentsを優先する
7. SLDS v1を利用する場合はSLDSクラスを独自CSSで直接overrideしない
...
のではなく、「27. 推奨アーキテクチャ」の全体像に沿って、基本はカスタムCSSによるデザイン実装（SLDS v1は必要時のみ部分利用）から始まりLWC化までを見据えた中間成果物として静的UIを作る。そのため、ビルド環境は可能な限り薄く保つ（3章「HTML共通化は『ビルド時Include』で行う」を参照）。

この方式により、

- フレームワーク依存を避けられる
- Component単位でLWCへ移行しやすい
- デザイン・SLDS利用箇所のレビューをComponent単位で実施できる
- 別ベンダーとの責任分界が明確になる
- LWC化後の表示差分を追跡しやすい
...
**「カスタムCSSによるデザイン実装（SLDS v1は利用可能な状態を維持） + Component単位管理 + Build時HTML Include + LWC境界を意識したCSS / JavaScript設計」**
