...
## 概要
UIデザインおよび静的フロントエンドの制作（Salesforce標準デザイン（SLDS）を考慮）を行い、LWC化担当の別ベンダーへ引き継ぎを行う作業が主となるプロジェクト。
...
|---|---|
| `src/pages` | 画面全体の構成 |
| `src/components` | 再利用パーツ、将来のLWC化候補 |
...
<body>
  ...
  <main class="app-main">
    ...
    <include src="../components/breadcrumb/breadcrumb.html"></include>
...
components/search-form/search-form.html
components/result-table/result-table.html
...
## 6. SLDS Blueprintを静的HTMLの基準とする

静的HTMLでは、SLDSを準拠したカスタムコンポーネントを構築するための設計図（Blueprint）を基本とする。
...
見た目だけ合わせるのではなく、HTMLの意味構造も正しくする。
...
SLDS Blueprintにアクセシビリティ要件が記載されている場合は、それも実装する。
...
PR作成時にはCIでも同様のチェックを行う。(CIの利用が可能であれば)
CI利用不可であれば、PR前に確認の徹底とPRレビュー時にLint検出も含め確認する。
...
Lintで検出できる問題を人間のPRレビューに持ち込まない。（手間を減らす）
...
5. SLDS Blueprintを静的HTML実装の基準にする
